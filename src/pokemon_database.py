...
class PokemonDatabase:
    ...
    def load(self):
        ...
        if not os.path.exists(config.POKEMON_DB_PATH):
            raise FileNotFoundError(
                f"pokemon_db.json not found at {config.POKEMON_DB_PATH}"
            )
    ...
    def _preload_sprites(self):
        ...
        for pkey, p in self.pokemon_db.items():
            ...
            p["description"] = (
                p.get("description")
                or p.get("desc")
                or p.get("flavor_text")
                or "No description available."
            )

    def _load_gen3_sprites(self, pokemon_data, pid_str):
        """Load Gen3 static sprites"""
        gen3_path = os.path.join(
            config.DATA_DIR, "sprites", "gen3", "normal", f"{pid_str}.png"
        )
        gen3_shiny_path = os.path.join(
            config.DATA_DIR, "sprites", "gen3", "shiny", f"{pid_str}.png"
        )

        pokemon_data["gen3_normal_path"] = (
            gen3_path if os.path.exists(gen3_path) else None
        )
        pokemon_data["gen3_shiny_path"] = (
            gen3_shiny_path if os.path.exists(gen3_shiny_path) else None
        )

        if pokemon_data["gen3_normal_path"]:
            try:
                pokemon_data["image"] = pygame.image.load(
                    pokemon_data["gen3_normal_path"]
                ).convert_alpha()
            except Exception:
    ...
    def _load_showdown_sprites(self, pokemon_data, pid_str):
        ...
        sd_normal = os.path.join(
            config.DATA_DIR, "sprites", "showdown", "normal", f"{pid_str}.gif"
        )
        sd_shiny = os.path.join(
            config.DATA_DIR, "sprites", "showdown", "shiny", f"{pid_str}.gif"
        )
            ...
            try:
                ...
                for frame in ImageSequence.Iterator(pil_img):
                    ...
                    durations.append(
                        ...
                        else config.SHOWDOWN_FRAME_MS_DEFAULT
                    )
                if frames:
    ...
    def _load_single_frame(self, pokemon_data, gif_path):
        ...
        try:
            ...
            pokemon_data["_showdown_frame_durations"] = [
                config.SHOWDOWN_FRAME_MS_DEFAULT
            ]
        except Exception:
            pokemon_data["_showdown_frames"] = None
            pokemon_data["_showdown_frame_durations"] = None
