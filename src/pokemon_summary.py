...
# Import move data
try:
    from move_data import get_move_name, get_move_info
...
# Import ability data
try:
    from ability_data import get_ability_name, get_pokemon_ability_name, get_pokemon_abilities, get_ability_description, get_pokemon_ability_id
...
class PokemonOptionsMenu:
    ...
    def __init__(self, pokemon, x, y, font, close_callback, 
                 summary_callback=None, move_callback=None, item_callback=None,
                 game_type='RSE'):
    ...
    def draw(self, surf):
        ...
        for i, option in enumerate(self.options):
            ...
            if i == self.selected_index:
                highlight_rect = pygame.Rect(self.rect.left + 4, y - 2, 
                                            self.width - 8, self.item_height - 4)
            ...
            text_rect = text_surf.get_rect(midleft=(self.rect.left + 12, y + self.item_height // 2 - 2))
...
class PokemonSummary:
    ...
    def __init__(self, pokemon, width, height, font, close_callback, 
                 manager=None, game_type='RSE', prev_pokemon_callback=None, 
        ...
        # Calculate layout
        self.tab_height = 30
        self.content_rect = pygame.Rect(0, self.tab_height, width, height - self.tab_height)
    ...
    def _load_sprite(self):
        ...
        if self.manager:
            try:
                ...
                print(f"[DEBUG] _load_sprite: manager.get_gen3_sprite_path returned {sprite_path} for pokemon {self.pokemon}")
        ...
        print(f"[PokemonSummary] Could not load sprite. Pokemon data keys: {list(self.pokemon.keys())}")
        print(f"[PokemonSummary] Species value: {self.pokemon.get('species', 'MISSING')}")
    ...
    def _draw_info_page(self, surf):
        ...
        display_name = nickname if nickname and nickname != species_name else species_name
        ...
        if self.sprite:
            scaled_sprite = pygame.transform.scale(self.sprite, (sprite_size - 4, sprite_size - 4))
        ...
        # Name - BIGGER font
        name_text = self.font.render(display_name.upper()[:10], True, ui_colors.COLOR_TEXT)
        ...
        for stat_key, stat_name, base_key in stats_list:
            ...
            stat_val = self.pokemon.get(stat_key) or self.pokemon.get('stats', {}).get(stat_key, 0)
            ...
            stat_label = self.small_font.render(f"{stat_name}:", True, ui_colors.COLOR_TEXT)
            surf.blit(stat_label, (col1_x, stats_y))
            
            # Stat value (right aligned to fixed position)
            stat_val_text = self.small_font.render(f"{stat_val:3d}", True, ui_colors.COLOR_TEXT)
            surf.blit(stat_val_text, (stat_val_x, stats_y))
            
            # IV/EV
            iv_ev_text = self.small_font.render(f"{iv:2d}/{ev:3d}", True, (120, 120, 120))
        ...
        ability_text = self.small_font.render(f"{ability_name}", True, ui_colors.COLOR_TEXT)
    ...
    def _draw_moves_page(self, surf):
        ...
        for i in range(4):
            ...
            if move and move.get('id', 0) > 0:
                ...
                # Move name
                name_text = self.small_font.render(move_name.upper(), True, ui_colors.COLOR_TEXT)
                ...
                type_text = self.small_font.render(move_type.upper()[:5], True, (255, 255, 255))
                type_text_rect = type_text.get_rect(center=type_rect.center)
                surf.blit(type_text, type_text_rect)
                
                # Power/Accuracy
                if power > 0:
                    pow_text = self.small_font.render(f"POW:{power}", True, (180, 180, 180))
                    surf.blit(pow_text, (move_rect.left + 70, move_rect.top + 22))
                
                # PP
                pp_text = self.small_font.render(f"PP {move_pp}/{max_pp}", True, (180, 180, 180))
    ...
    def _draw_contest_page(self, surf):
        ...
        # Sheen value text
        sheen_text = self.small_font.render(str(sheen_value), True, ui_colors.COLOR_TEXT)
