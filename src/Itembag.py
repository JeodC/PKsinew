...
class Modal:
    ...
    def draw(self, surf):
        ...
        pygame.draw.rect(
            surf, ui_colors.COLOR_BORDER, (0, 0, self.width, self.height), 3
        )

        # Inner border
        pygame.draw.rect(
            surf, (70, 70, 90), (6, 6, self.width - 12, self.height - 12), 2
        )
...
class ItemBagScreen:
        ...
        if ctrl.is_dpad_just_pressed("down"):
            ...
            if self.selected_item_index < len(items) - 1:
                ...
                if self.selected_item_index > last_visible:
                    self.scroll_offset = (
                        self.selected_item_index - self.items_per_page + 1
                    )
            consumed = True
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    ...
                    self.select_pocket(
                        (self.current_pocket_index - 1) % len(self.pocket_order)
                    )
            ...
            # Mouse wheel scrolling (scrolls view but doesn't change selection)
                ...
                if event.button == 4:  # Scroll up
                    ...
                    if (
                        ...
                        self.selected_item_index = (
                            self.scroll_offset + self.items_per_page - 1
                        )
    ...
    def draw(self, surf):
        ...
        if not items:
            # No items message
            no_items_surf = self.font_text.render(
                "No items in this pocket", True, (150, 150, 150)
            )
            ...
            for i, item in enumerate(visible_items):
                ...
                item_rect = pygame.Rect(
                    list_area.x + 5, y_pos, list_area.width - 10, item_height
                )

                # Check if this item is selected
                is_selected = (
                    actual_index == self.selected_item_index
                    and self.focus_mode == "items"
                )
                ...
                if is_selected:
                    ...
                    arrow_surf = self.font_text.render(
                        ">", True, ui_colors.COLOR_HIGHLIGHT
                    )
                    surf.blit(arrow_surf, (item_rect.x - 12, item_rect.y + 3))

                # Item name
                name_surf = self.font_text.render(
                    item["name"][:25], True, ui_colors.COLOR_TEXT
                )
                ...
                qty_rect = qty_surf.get_rect(
                    right=item_rect.right - 5, centery=item_rect.centery
                )
