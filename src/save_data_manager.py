...
try:
    ...
    from parser.trainer import format_trainer_id, format_play_time
...
class SaveDataManager:
    ...
    def format_trainer_id(self, show_secret=False):
        ...
        return format_trainer_id(
            self.parser.trainer_id,
            self.parser.secret_id,
            show_secret
        )
    ...
    def get_box(self, box_number):
        ...
        # Enrich with species names
        return [self._enrich_pokemon(p.copy() if p and not p.get('empty') else p) for p in box]
    ...
    def format_play_time(self):
        ...
        return format_play_time(
            self.parser.play_hours,
            self.parser.play_minutes,
            self.parser.play_seconds
        )
