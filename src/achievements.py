...
class AchievementNotification:
    ...
    def update(self):
        ...
        if self.state == self.STATE_SLIDING_IN:
            ...
            self.y_offset = -self.NOTIFICATION_HEIGHT + (
                self.NOTIFICATION_HEIGHT * progress
            )
    ...
    def draw(self, surf):
        ...
        pygame.draw.rect(
            surf, (255, 215, 0), banner_rect, 3, border_radius=8
        )  # Gold border
...
class AchievementManager:
    ...
    def __init__(self):
        self.progress = (
            {}
        )  # {achievement_id: {"unlocked": bool, "unlocked_at": timestamp, "reward_claimed": bool}}
        self.stats = {}  # Global stats like total_transfers, total_sinew_stored, etc.
        self.tracking = (
            {}
        )  # PER-GAME tracking values {game_name: {hint_key: current_value}}
        self.high_water_marks = (
            {}
        )  # High water marks for values that should only go up {game: {key: max_value}}
        self.altering_cave_claimed = (
            []
        )  # List of species IDs claimed from Altering Cave slot machine
    ...
    def claim_altering_cave_pokemon(self, species_id):
        ...
        print(
            f"[Achievements] Altering Cave Pokemon claimed: species {species_id} ({count}/7)"
        )
        return True
    ...
    def force_check_by_tracking(self, game_name=None):
        ...
        for game in games_to_check:
            ...
            if game == "Sinew":
                game_tracking = self.tracking.get(
                    "Sinew", self.tracking.get("global", {})
                )
            else:
            ...
            for ach in achievements:
                    ...
                    try:
                        ...
                        if "money" in key.lower():
                            ...
                            if tracking_val > current:
                                current = self.update_high_water_mark(
                                    key, tracking_val, game
                                )
                        else:
                ...
                if "owns_species_" in hint and "owns_species_380_or_381" not in hint:
                    try:
                        species_id = int(
                            hint.split("owns_species_")[1].split("_")[0].split()[0]
                        )
                        if game == "Sinew":
                ...
                if "owns_species_380_or_381" in hint:
                    ...
                    if 380 in owned or 381 in owned:
                        unlocked = True
                        print(
                            f"[Achievements] Force unlock: {ach['name']} (Latias/Latios in {game})"
                        )
                ...
                if "owns_regi_trio" in hint:
                    ...
                    if 377 in owned and 378 in owned and 379 in owned:
                        unlocked = True
                        print(
                            f"[Achievements] Force unlock: {ach['name']} (Regi trio in {game})"
                        )
                ...
                if "owns_weather_trio" in hint:
                    ...
                    if 382 in owned and 383 in owned and 384 in owned:
                        unlocked = True
                        print(
                            f"[Achievements] Force unlock: {ach['name']} (Weather trio in {game})"
                        )
        ...
        for game in GAMES + ["Sinew", "global"]:
            ...
            if game_tracking:
                ...
                if isinstance(owned_set, set) and owned_set:
                    ...
                    if found:
                        print(
                            f"[Achievements]     Legendaries in {game} owned_set: {found}"
                        )
            ...
            for ach in achievements:
                ...
                if progress:
                    ...
                    if percentage >= 100:
                        ...
                        print(
                            f"[Achievements]   progress: {current}/{required} ({percentage}%)"
                        )
        ...
        if "owns_species_" in hint:
            ...
            try:
                ...
                if ach_game == "Sinew":
                    ...
                    sinew_tracking = self.tracking.get(
                        "Sinew", self.tracking.get("global", {})
                    )
                    owned = sinew_tracking.get("combined_pokedex_set", set())
                else:
    ...
    def _load_progress(self):
        ...
        try:
            if os.path.exists(ACH_SAVE_PATH):
                ...
                print(
                    f"[Achievements] Loaded progress: {len(self.progress)} achievements tracked"
                )
    ...
    def unlock_achievement(self, achievement_data):
        ...
        return self.unlock(
            ach_id, achievement_data if isinstance(achievement_data, dict) else None
        )
                    ...
                    if box:
                        for slot_idx, slot in enumerate(box):
                            ...
                            if is_empty:
                                ...
                                success = sinew.set_pokemon_at(
                                    box_num, slot_idx, pokemon_dict
                                )
                                if success:
                                    print(
    ...
    def _parse_pks_file(self, data):
        ...
        try:
            ...
            print(
                f"[Achievements] Parsed Pokemon: {pokemon['species_name']} Lv.{pokemon['level']}"
            )
            return pokemon
    ...
    def reset_achievement(self, achievement_id):
        ...
        if achievement_id in self.progress:
            ...
            if (
                "altering_cave" in achievement_id.lower()
                or self._is_altering_cave_achievement(achievement_id)
            ):
                self.altering_cave_claimed = []
                # Also reset tracking
    ...
    def _is_altering_cave_achievement(self, achievement_id):
        ...
        try:
            sinew_achs = get_achievements_for("Sinew")
            for ach in sinew_achs:
                if ach["id"] == achievement_id and "altering_cave_echoes" in ach.get(
                    "hint", ""
                ):
                    return True
        except Exception:
    ...
    def force_reload(self):
        ...
        self._load_progress()
        print(
            f"[Achievements] Force reloaded - altering_cave_claimed={self.altering_cave_claimed}"
        )
    ...
    def revalidate_achievements(self):
            ...
            for ach in achievements:
                ...
                # Check == True style hints
                    ...
                    try:
                        ...
                        if key in ("dev_mode_activated", "debug_test_activated"):
                            ...
                            should_be_unlocked = (
                                True  # Can't validate without tracking data
                            )
                        else:
                ...
                # Check species ownership
                    ...
                    if game == "Sinew":
                        owned = sinew_tracking.get("combined_pokedex_set", set())
                        should_be_unlocked = (
                            377 in owned and 378 in owned and 379 in owned
                        )
                    elif not game_has_tracking:
                        should_be_unlocked = True
                    else:
                    ...
                    if game == "Sinew":
                        owned = sinew_tracking.get("combined_pokedex_set", set())
                        should_be_unlocked = (
                            382 in owned and 383 in owned and 384 in owned
                        )
                    elif not game_has_tracking:
                        should_be_unlocked = True
                    else:
                ...
                if not should_be_unlocked:
                    print(
                        f"[Achievements] REVOKING: {ach['name']} ({game}) - hint: {hint}"
                    )
                    del self.progress[ach["id"]]
        ...
        if revoked:
            self._save_progress()
            print(
                f"[Achievements] Revoked {len(revoked)} incorrectly unlocked achievements"
            )
        else:
    ...
    def check_and_unlock(self, save_data, game_name, sinew_data=None):
        ...
        if money_hwm > money:
            print(
                f"[Achievements]   Using money high water mark: {money_hwm} (current: {money})"
            )
        ...
        print(
            f"[Achievements]   dex_caught={dex}, money={money} (hwm={money_hwm}), badges={badges}"
        )
        print(
            f"[Achievements]   pc_pokemon={pc_len}, party={party_len}, playtime={playtime:.1f}h"
        )
        ...
        for ach in game_achievements:
            if not self.is_unlocked(ach["id"]):
                ...
                if "pc_pokemon" in hint or "money >=" in hint or "dex_count >=" in hint:
                    print(
                        f"[Achievements]   Checking {ach['name']}: {hint} -> {result}"
                    )
                if result:
                    if self.unlock(
                        ach["id"], ach
                    ):  # Pass achievement data for notification
                        newly_unlocked.append(ach["id"])
            else:
                already_unlocked += 1
        ...
        if sinew_data:
            ...
            sinew_save_data = sinew_data.get(
                "aggregate", {}
            )  # Use aggregate, not per-game data
    ...
    def get_points(self, game_name=None):
        ...
        return sum(
            a.get("points", 0) for a in achievements if self.is_unlocked(a["id"])
        )

    def check_sinew_achievements(
        self,
        sinew_storage_count=None,
        transfer_count=None,
        shiny_count=None,
        evolution_count=None,
    ):
        """
...
class AchievementsScreen:
    ...
    def _load_achievements(self):
        ...
        def sort_key(a):
            ...
            should_show = (
                self.manager.should_show_reward(ach_id) if has_reward else False
            )
            has_unclaimed_gift = (
                is_unlocked and has_reward and not reward_claimed and should_show
            )

            if has_unclaimed_gift:
                # Priority 0: unclaimed gifts at very top
                unlock_time = self.manager.progress.get(ach_id, {}).get(
                    "unlocked_at", 0
                )
        ...
        if self.tab_focus:
            ...
            if ctrl.is_dpad_just_pressed("down"):
                ...
                if self.selected_achievement < len(self.achievements) - 1:
                    self.selected_achievement += 1
                    if (
                        self.selected_achievement
                        >= self.scroll_offset + self.achievements_per_page
                    ):
                        self.scroll_offset = (
                            self.selected_achievement - self.achievements_per_page + 1
                        )
                consumed = True
    ...
    def draw(self, surf, font):
        ...
        if not self.achievements:
            ...
            visible_achievements = self.achievements[
                self.scroll_offset : self.scroll_offset + self.achievements_per_page
            ]
            ...
            for i, achievement in enumerate(visible_achievements):
                ...
                is_selected = (
                    actual_index == self.selected_achievement
                ) and not self.tab_focus
                ...
                should_show = (
                    self.manager.should_show_reward(achievement["id"])
                    if has_reward
                    else False
                )
                has_unclaimed_gift = (
                ...
                if icon:
                    ...
                    if not is_unlocked:
                        dimmed = icon.copy()
                        dimmed.fill(
                            (100, 100, 100, 180), special_flags=pygame.BLEND_RGBA_MULT
                        )
                ...
                if is_unlocked:
                    ...
                    if progress:
                        ...
                        progress_surf = self.font_small.render(
                            progress_text, True, (150, 150, 150)
                        )
                ...
                if has_reward:
                    ...
                    if reward_info:
                        ...
                        if reward_type == "both":
                            ...
                            reward_name = (
                                f"Unlock: {reward_info.get('name', 'Feature')}"
                            )
                        else:
                        ...
                        if has_unclaimed_gift:
                            ...
                            gift_surf = self.font_small.render(
                                gift_text, True, gift_color
                            )
                            surf.blit(
                                gift_surf,
                                (
                                    box_rect.right - gift_surf.get_width() - 5,
                                    box_rect.y + 18,
                                ),
                            )
        ...
        if len(self.achievements) > self.achievements_per_page:
            ...
            if self.scroll_offset < max_scroll:
                ...
                surf.blit(
                    ...
                    (
                        self.width - 20,
                        y_start + self.achievements_per_page * item_height - 15,
                    ),
                )
    ...
    def _draw_detail_popup(self, surf):
        ...
        should_show = (
            self.manager.should_show_reward(ach["id"]) if has_reward else False
        )
        ...
        if not is_unlocked:
            ...
            if progress:
                ...
                # Progress label
                progress_label = self.font_small.render(
                    "Progress:", True, (150, 150, 100)
                )
                ...
                progress_surf = self.font_small.render(
                    progress_text, True, (200, 200, 200)
                )
        ...
        if category:
            cat_surf = self.font_small.render(
                f"Category: {category}", True, (150, 150, 150)
            )
        ...
        if has_reward:
            ...
            if not is_unlocked:
                ...
                reward_surf = self.font_small.render(
                    f"[LOCKED] {reward_desc}", True, (120, 120, 120)
                )
                ...
                reward_surf = self.font_small.render(
                    f"[CLAIMED] {reward_desc}", True, (150, 255, 150)
                )
                surf.blit(reward_surf, (popup_x + 15, reward_y))
            elif not should_show:
                # Theme already unlocked from another achievement
                reward_surf = self.font_small.render(
                    f"[OWNED] {reward_desc}", True, (150, 255, 150)
                )
                surf.blit(reward_surf, (popup_x + 15, reward_y))
            else:
        ...
        if has_reward and is_unlocked and should_show:
            close_hint = self.font_small.render(
                "A = Claim | B = Close", True, (100, 100, 100)
            )
        else:
            close_hint = self.font_small.render(
                "Press A or B to close", True, (100, 100, 100)
            )
        ...
        if self._reward_message and current_time - self._reward_message_time < 3000:
            ...
            msg_surf = self.font_text.render(
                self._reward_message, True, (255, 255, 255)
            )
            msg_rect = msg_surf.get_rect(
                centerx=popup_x + popup_w // 2, centery=popup_y + popup_h // 2
            )
