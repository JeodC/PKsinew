...
class PCBox:
    ...
    def _execute_undo(self):
        ...
        from save_writer import clear_pc_slot, write_pokemon_to_pc, write_save_file
    ...
    def get_party_slot_rects(self, inner_y=None, inner_height=None):
        ...
        base_w = (
            panel_rect.width - slot_margin * 4
        ) / 3  # width for left + right slots
        base_h = (inner_height - slot_margin * 6) / 6  # height for 6 slots
        slot_size = (
            min(base_w, base_h) * self.party_slot_scale
        )  # make square and apply scale
    ...
    def _complete_altering_cave_exchange(self):
        ...
        try:
            ...
            print(
                f"[PCBox] Generated {result_pokemon['name']}: {len(pks_data)} bytes, species={pokemon_dict.get('species')}"
            )
            ...
            self.warning_message_timer = int(
                self.warning_message_duration * 1.25
            )  # Show slightly longer
