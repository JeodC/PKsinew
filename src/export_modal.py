...
try:
    from pokemon_summary import calculate_hp, get_base_stats
...
def sanitize_for_json(obj):
    ...
    if isinstance(obj, bytes):
        ...
        return {
            ...
            and k not in ("raw_data", "raw_bytes", "data", "encrypted_data")
        }
...
class ExportModal:
    ...
    def __init__(self, width, height, game_name=None, close_callback=None):
        ...
        self.game_type = (
            "FRLG" if "Fire" in self.game_name or "Leaf" in self.game_name else "RSE"
        )
    ...
    def _load_save_data(self):
        ...
        try:
            ...
            try:
                items = (
                    manager.get_items_with_names()
                    if hasattr(manager, "get_items_with_names")
                    else []
                )
                if items:
                    data["items"] = items
        ...
        if moves:
            ...
            for move in moves:
                if isinstance(move, dict):
                    ...
                    result["moves"].append({"name": get_move_name(move), "id": move})
        ...
        if self.tab_focus:
            ...
            if self.selected_tab == 2:
                ...
                if ctrl.is_dpad_just_pressed("down"):
                    ctrl.consume_dpad("down")
                    self.selected_option = (self.selected_option + 1) % (
                        len(self.export_options) + 1
                    )
                    consumed = True

                if ctrl.is_dpad_just_pressed("left") or ctrl.is_dpad_just_pressed(
                    "right"
                ):
                    ctrl.consume_dpad("left")
                    ctrl.consume_dpad("right")
                    if self.selected_option < len(self.export_options):
                        self.export_options[self.selected_option]["value"] = (
                            not self.export_options[self.selected_option]["value"]
                        )
                    consumed = True
                ...
                if ctrl.is_dpad_just_pressed("up"):
                    ...
                    if self.scroll_positions[self.selected_tab] == 0:
                        ...
                        self.scroll_positions[self.selected_tab] = (
                            self.scroll_positions[self.selected_tab] - 1
                        )
                    consumed = True
                ...
                if ctrl.is_dpad_just_pressed("down"):
                    ctrl.consume_dpad("down")
                    max_scroll = max(
                        0, len(self.tab_content.get(self.selected_tab, [])) - 15
                    )
                    self.scroll_positions[self.selected_tab] = min(
                        max_scroll, self.scroll_positions[self.selected_tab] + 1
                    )
                    consumed = True
    ...
    def _do_export(self):
        ...
        include_trainer = any(
            o["key"] == "trainer" and o["value"] for o in self.export_options
        )
        include_items = any(
            o["key"] == "items" and o["value"] for o in self.export_options
        )
        include_party = any(
            o["key"] == "party" and o["value"] for o in self.export_options
        )
        include_box = any(o["key"] == "box" and o["value"] for o in self.export_options)
        include_detailed = any(
            o["key"] == "detailed" and o["value"] for o in self.export_options
        )

        if include_trainer:
            export_data["trainer"] = sanitize_for_json(
                self.save_data.get("trainer", {})
            )
        ...
        if include_party:
            ...
            if include_detailed:
                export_data["party"] = [
                    ...
                    for p in party
                    if isinstance(p, dict) and not p.get("empty")
                ]
            else:
                export_data["party"] = [
    ...
    def _draw_export_options(self, surf, rect):
        ...
        is_btn_selected = not self.tab_focus and self.selected_option == len(
            self.export_options
        )
