...
class ControllerManager:
    ...
    def _apply_profile_for_controller(self, joy):
        ...
        # Capture hardware originals BEFORE applying to button_map
        # so swap logic always knows the true hardware A/B
        self._original_a = mapping.get("A", [0])
        self._original_b = mapping.get("B", [1])
        if isinstance(self._original_a, int):
            self._original_a = [self._original_a]
        if isinstance(self._original_b, int):
            self._original_b = [self._original_b]
    ...
    def _store_originals_and_swap(self):
        ...
        hardware values.
        """
        if not getattr(self, "_original_a", None):
            self._original_a = self.button_map["A"][:]
        if not getattr(self, "_original_b", None):
            self._original_b = self.button_map["B"][:]

        # Check both persistent and pending swap preference
    ...
    def _scan_controllers(self):
        ...
        for i in range(count):
            try:
                ...
                # Reject devices with no buttons/axes (haptic motors, power keys, etc.)
                name = joy.get_name().lower()
                is_system_device = any(
                    x in name for x in ["haptics", "pwrkey", "resin", "headset"]
                )

                if joy.get_numbuttons() == 0 or joy.get_numaxes() == 0 or is_system_device:
    ...
    def resume(self):
        """Reclaims the joystick hardware and restores swap state."""
        print("[Controller] Resuming input for Sinew...")
        current_swap_state = getattr(self, "_swap_ab", False)
        pygame.joystick.init()
        self._scan_controllers()
        # Restore swap state that may have been lost during re-init
        if current_swap_state:
