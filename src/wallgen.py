...
from config import SPRITES_DIR, FONT_SOLID_PATH
...
def draw_vignette(img):
    ...
    for i in range(steps):
        ...
        bbox = [int(cx - cx*(i/steps)), int(cy - cy*(i/steps)),
                int(cx + cx*(i/steps)), int(cy + cy*(i/steps))]
...
def paste_logo_or_text(bg, name):
    if name.lower() == "sinew":
        ...
        outline_width = 2
        text_layer = Image.new("RGBA", (text_w + outline_width*2, text_h + outline_width*2), (0,0,0,0))
        draw = ImageDraw.Draw(text_layer)

        # Black outline
        offsets = [(dx, dy) for dx in range(-outline_width, outline_width+1)
                             for dy in range(-outline_width, outline_width+1) if dx != 0 or dy != 0]
        for ox, oy in offsets:
            draw.text((outline_width + ox - bbox[0], outline_width + oy - bbox[1]),
                      name, font=font, fill=(0,0,0,255))

        # Main white text
        draw.text((outline_width - bbox[0], outline_width - bbox[1]), name, font=font, fill=(255,255,255,255))
