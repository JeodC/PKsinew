...
class ButtonMapper:
                        ...
                        if isinstance(binding, dict):
                            ...
                            if source == "button":
                                ...
                                if direction in ("left", "right"):
                                    pair = (
                                        axis_idx,
                                        axis_idx + 1 if axis_idx % 2 == 0 else axis_idx,
                                    )
                                else:
                                    pair = (
                                        axis_idx - 1 if axis_idx % 2 == 1 else axis_idx,
                                        axis_idx,
                                    )
                    ...
                    if has_dpad_config:
                        ...
                        for dpad_key in [
                            ...
                            if isinstance(self.mapping.get(dpad_key), dict):
                                btn_mapping["_dpad_bindings"][dpad_key] = self.mapping[
                                    dpad_key
                                ]
    ...
    def _apply_mapping_to_controller(self):
        ...
        for btn in ["A", "B", "L", "R", "SELECT", "START"]:
            if btn in self.mapping:
                ...
                if isinstance(val, list):
                    self.controller.button_map[btn] = [
        ...
        print(
            f"[ButtonMapper] Applied mapping to controller "
    ...
    def _stop_listening(self, new_binding=None):
        ...
        if new_binding is not None and self.listening_button:
            ...
            if is_dpad and isinstance(new_binding, dict):
                ...
                if dup:
                    print(
                        f"[ButtonMapper] Button {new_binding} already bound to {dup}!"
                    )
                    self.mapping[dup] = []
                ...
                self.mapping[self.listening_button] = [new_binding]
                print(
                    f"[ButtonMapper] Bound {self.listening_button} to button {new_binding}"
                )
                self._show_status(
                    f"{self.listening_button} -> Btn {new_binding}", (100, 255, 150)
                )
                ...
                self.mapping[self.listening_button] = {
                    "source": "button",
                    "button": new_binding,
                }
                self._show_status(
                    f"{self.listening_button.replace('DPAD_', '')} -> Btn {new_binding}",
                    (100, 255, 150),
                )
                print(
                    f"[ButtonMapper] Bound {self.listening_button} to button {new_binding}"
    ...
    def handle_events(self, events):
        ...
        for event in events:
            ...
            if self.listening:
                ...
                elif event.type == pygame.JOYHATMOTION and getattr(
                    self, "listening_is_dpad", False
                ):
                    ...
                    if hx != 0 or hy != 0:
                        ...
                        if abs(hx) >= abs(hy) and hx != 0:
                            ...
                            binding = {
                                ...
                                "hat": event.hat,
                                "axis": "y",
                                "value": 1 if hy > 0 else -1,
                            }
                        self._stop_listening(binding)

                elif event.type == pygame.JOYAXISMOTION and getattr(
                    self, "listening_is_dpad", False
                ):
    ...
    def _draw_profile_info(self, surf, y):
        ...
        if self._profile_info and self._profile_info.get("description"):
            ...
            info_surf = self.font_tiny.render(
                self._controller_name, True, (150, 150, 150)
            )
            info_rect = info_surf.get_rect(centerx=self.width // 2, top=y)
            surf.blit(info_surf, info_rect)
    ...
    def _draw_gba(self, surf):
        ...
        for i in range(4):
            ...
            grill_rect = pygame.Rect(self.gba_rect.right - 35, grill_y, 20, 3)
        ...
        for btn_name, rect in self.button_rects.items():
            is_selected = (
                not self.in_menu and self.button_list[self.selected_index] == btn_name
            )
            ...
            if btn_name in ("A", "B"):
                ...
                if is_selected or is_listening:
                    pygame.draw.rect(
                        surf, bg_color, rect.inflate(2, 2), border_radius=2
                    )
                    pygame.draw.rect(
                        surf, border_color, rect.inflate(2, 2), 2, border_radius=2
                    )
            ...
            if not is_dpad:
                ...
                if btn_name in ("L", "R"):
                    label_rect = label_surf.get_rect(
                        centerx=rect.centerx, top=rect.bottom + 2
                    )
                elif btn_name in ("A", "B"):
                    label_rect = label_surf.get_rect(
                        centerx=rect.centerx, top=rect.bottom + 3
                    )
                else:
                    label_rect = label_surf.get_rect(
    ...
    def _draw_screen_content(self, surf):
        ...
        if self.listening:
            ...
            btn_rect = btn_surf.get_rect(
                centerx=self.screen_rect.centerx, centery=self.screen_rect.centery - 25
            )
            ...
            hint_rect = hint_surf.get_rect(
                centerx=self.screen_rect.centerx, centery=self.screen_rect.centery - 10
            )
            surf.blit(hint_surf, hint_rect)
            ...
            count_rect = count_surf.get_rect(
                centerx=self.screen_rect.centerx, centery=self.screen_rect.centery + 5
            )
            ...
            # Show status message
            status_surf = self.font_text.render(
                self.status_message, True, self.status_color
            )
            status_rect = status_surf.get_rect(
                centerx=self.screen_rect.centerx, centery=self.screen_rect.centery
            )
            ...
            for line in lines:
                if line:
                    ...
                    line_rect = line_surf.get_rect(
                        centerx=self.screen_rect.centerx, top=y
                    )
