...
class ThemesScreen:
        ...
        if ACHIEVEMENTS_AVAILABLE:
            ...
            if manager:
                ...
                if required_ach_id.startswith("*"):
                    ...
                    from achievements_data import GAME_PREFIX, GAMES
    ...
    def handle_controller(self, ctrl):
        ...
        if ctrl.is_button_just_pressed("R"):
            ...
            self.selected_index = min(
                max_idx, self.selected_index + self.themes_per_page
            )
            max_scroll = max(0, len(self.themes) - self.themes_per_page)
            self.scroll_offset = min(
                max_scroll, self.scroll_offset + self.themes_per_page
            )
            consumed = True
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    if self.selected_index < len(self.themes) - 1:
                        ...
                        if (
                            self.selected_index
                            >= self.scroll_offset + self.themes_per_page
                        ):
                            self.scroll_offset = (
    ...
    def draw(self, surf):
        ...
        visible_themes = self.themes[
            self.scroll_offset : self.scroll_offset + self.themes_per_page
        ]
        ...
        for i, theme_name in enumerate(visible_themes):
            ...
            if is_selected:
                ...
                # Cursor
                cursor_color = (
                    (150, 100, 100) if is_locked else ui_colors.COLOR_HIGHLIGHT
                )
                cursor = self.font_text.render(">", True, cursor_color)
                surf.blit(cursor, (5, y + 12))
            elif is_current:
                pygame.draw.rect(
                    surf, ui_colors.COLOR_SUCCESS, item_rect, 2, border_radius=4
                )
            else:
        ...
        preview_rect = pygame.Rect(
            preview_x, y_start, preview_width, self.height - y_start - 40
        )
        ...
        if preview:
            ...
            header_rect = pygame.Rect(
                preview_x + 5, y_start + 5, preview_width - 10, 25
            )
                ...
                if hint:
                    ...
                    if is_theme_locked:
                        ...
                        lines = self._wrap_text(
                            "LOCKED - " + hint, self.font_small, max_width
                        )
                        text_color = (180, 120, 120)
                    else:
                        # Show how it was unlocked (unlocked)
                        lines = self._wrap_text(
                            hint + " (Unlocked)", self.font_small, max_width
                        )
            ...
            pygame.draw.rect(
                surf, ui_colors.COLOR_BUTTON, preview_rect, border_radius=4
            )
        ...
        if len(self.themes) > self.themes_per_page:
            ...
            if self.scroll_offset < len(self.themes) - self.themes_per_page:
                ...
                surf.blit(
                    down_arrow,
                    (list_width // 2, y_start + self.themes_per_page * item_height - 5),
                )
        ...
        if self._lock_message and current_time - self._lock_message_time < 2000:
            ...
            msg_rect = msg_surf.get_rect(
                centerx=self.width // 2, bottom=self.height - 30
            )
