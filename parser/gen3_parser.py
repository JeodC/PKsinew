...
from .save_structure import (
    find_active_save_slot, build_section_map, detect_game_type,
    get_save_info, validate_save
)
from .trainer import parse_trainer_info, format_trainer_id, format_play_time
from .pokemon import parse_party, parse_pc_boxes, get_box_structure
from .items import parse_bag, parse_money, get_bag_summary
...
class Gen3SaveParser:
    ...
    def load(self, save_path=None):
        ...
        try:
            ...
            self.game_type, self.game_name = detect_game_type(self.data, self.section_offsets)
    ...
    def get_party(self):
        ...
        if self._party is None:
            ...
            print(f"[Parser] Parsing party with game_type={self.game_type}, section1=0x{section1:X}")
            ...
            for i, poke in enumerate(self._party):
                print(f"[Parser]   {i+1}. #{poke.get('species', 0):03d} {poke.get('nickname', '???')} Lv.{poke.get('level', 0)}")
    ...
    def get_box_summary(self):
        ...
        for box_num in range(1, 15):
            ...
            empty = 30 - filled
            empty_slots = [slot['slot'] for slot in box_slots if slot.get('empty', False)]
    ...
    def get_bag(self):
        ...
        if not self.loaded:
            return {
                'items': [], 'key_items': [], 'pokeballs': [],
                'tms_hms': [], 'berries': []
            }
        
        if self._bag is None:
            section1 = self.section_offsets.get(1, 0)
            self._bag = parse_bag(self.data, section1, self.game_type, self.section_offsets)
    ...
    def money(self):
        ...
        if self._money is None:
            section1 = self.section_offsets.get(1, 0)
            self._money = parse_money(self.data, section1, self.game_type, self.section_offsets)
    ...
    def get_pokedex(self):
        ...
        """
        if not self.loaded:
            return {'owned_count': 0, 'seen_count': 0, 'owned_list': [], 'seen_list': []}
